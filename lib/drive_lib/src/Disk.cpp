...
void Disk::open(const std::string &path, size_t n_blocks) {
    ...
    if (ftruncate(file_descriptor_, n_blocks * K_Block_Size) < 0) {
...
void Disk::read(int block_num, char *data) {
    ...
    if (lseek(file_descriptor_, block_num * K_Block_Size, SEEK_SET) < 0) { // TODO : check static_casts here
    ...
    if (::read(file_descriptor_, data, K_Block_Size) != K_Block_Size) {
...
void Disk::write(int block_num, char *data) {
    ...
    if (lseek(file_descriptor_, block_num * K_Block_Size, SEEK_SET) < 0) {
    ...
    if (::write(file_descriptor_, data, K_Block_Size) != K_Block_Size) {
